...
@section('content')
<x-header :breadcrumbs="[['url' => route('team.index'), 'title' => 'Team'], ['title' => 'Create a team']]" :page-title="'Form'" />
...
<section class="col-start-2 col-end-7 row-start-1 row-end-7 justify-center mt-28">
    <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16 shadow-lg">
        ...
        <form method="POST" action="{{ route('team.store', ['serverId' => $server->id]) }}">
            @csrf
            ...
            <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                <div class="sm:col-span-2">
                    <label for="server_name" class="block mb-2 text-sm font-medium text-gray-900">
                        Team name
                        <input id="team_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" type="text" name="team_name" placeholder="Type team name" value="{{ old('team_name') }}" />
                    </label>
                    @error('team_name')
